  ...
  return (
    <div className="space-y-16 sm:space-y-24 animate-in fade-in duration-1000">
      <div className="text-center max-w-3xl mx-auto mb-12 sm:mb-16 px-4">
        ...
        <div className="w-24 sm:w-32 h-px bg-[#d4af37]/40 mx-auto"></div>
      </div>
      ...
      {factionNames.map(faction => (
        <section key={faction} className="relative">
          <div className="flex flex-col items-center mb-12 sm:mb-16 px-4">
            <h3 className="text-3xl sm:text-4xl font-display font-bold text-white tracking-[0.2em] mb-4 text-center break-keep">{faction}</h3>
          ...
          {/* 가문 목록 요약 */}
          <div className="grid sm:grid-cols-2 md:grid-cols-3 gap-4 sm:gap-6 mb-10 sm:mb-12 px-4 sm:px-0">
            {families[faction].map((fam, i) => (
              <div key={i} className="bg-stone-900/80 border border-stone-700 p-5 sm:p-6 text-center transform transition-all hover:-translate-y-1 hover:border-[#d4af37]/60 shadow-xl relative overflow-hidden">
                <div className="absolute top-0 left-1/2 -translate-x-1/2 w-1/2 h-1 bg-[#d4af37]/40 rounded-b"></div>
                <div className="text-xs sm:text-sm font-bold tracking-widest text-[#d4af37] font-serif mb-2">{fam.role} 세력</div>
                <div className="font-display font-bold text-white text-xl sm:text-2xl mb-3 sm:mb-4 tracking-wider">{fam.name} 가문</div>
                <div className="text-xs sm:text-sm text-stone-300 font-serif font-medium break-keep">
                  {fam.members.join(' · ')}
                </div>
              </div>
            ))}
            ...
            {npcs[faction].map((npc, idx) => (
              <div key={idx} className="group relative bg-[#0a0808] border border-stone-800 flex flex-col overflow-hidden transition-all duration-500 hover:border-[#d4af37]/50 hover:shadow-[0_0_30px_rgba(212,175,55,0.15)] shadow-xl">
                ...
                >
                  ...
                  {/* Name Overlay */}
                  <div className="absolute bottom-0 left-0 w-full p-4 sm:p-6 z-20 bg-gradient-to-t from-[#0a0808] via-[#0a0808]/90 to-transparent pointer-events-none">
                    <div className="flex justify-between items-end gap-2">
                      <div>
                        <div className="text-[#d4af37] font-serif font-bold text-[10px] sm:text-sm mb-1">{npc.title}</div>
                        <h4 className="text-2xl sm:text-3xl font-display font-bold text-white tracking-widest break-keep">{npc.name}</h4>
                      </div>
                      <div className={`px-2 sm:px-3 py-1 text-[10px] sm:text-xs font-bold font-serif whitespace-nowrap ${ npc.gender === '女' ? 'text-rose-400 border border-rose-900/50 bg-rose-950/80' : 'text-blue-400 border border-blue-900/50 bg-blue-950/80'}`}>
                        {npc.gender === '女' ? '여성' : '남성'}
                      </div>
                    </div>
                  </div>
